...
- **统一 Prompt 管理**: 使用 promptLoader 服务集中管理 Codex/Claude Code/Droid 系统提示词，支持多场景切换和三级优先级（用户自定义 > 配置默认 > 不注入）
...
- **promptLoader.js**: 统一 Prompt 加载器，启动时加载所有 prompts，支持场景切换
- **resources/codex-prompts/**: Codex 官方 Prompt 文件目录（default.txt 24KB、gpt-5-codex.txt 11KB、review.txt 6KB）
